...
First, install nnUNetv2 following the instructions in https://github.com/MIC-DKFZ/nnUNet.

Secondly, create folders called "raw", "pre", "models" and "output" inside of this current "nnunet" folder and run this in the terminal:
...
The model that we used is not public and it belongs to the BCN-AIM Lab, so we can't provide it here (also because it weighs 3GB+). 
The raw and pre folders can remain empty.

Finally, after having generated the "3DImagesPreFilter" folder in the "image_processing_and_tda" notebook, run the next command in the terminal.
The "cpu" can be changed to "gpu" or "cuda" if there are the conditions to do so.
